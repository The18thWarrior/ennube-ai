...
import { z } from 'zod';
import { VisualizationConfig, ComponentConfig, COMPONENT_TYPES, COMPONENT_CATEGORIES } from '../types';
 ...
 */
export function getComponentCategory(type: string): string | null {
 ...
 */
export function validateVisualizationConfig(config: any): { isValid: boolean; errors: string[]; sanitized?: VisualizationConfig } {
  const errors: string[] = [];
  ...
  try {
    ...
    const validatedComponents: ComponentConfig[] = [];
    ...
    const sanitized: VisualizationConfig = {
 ...
 */
function validateComponent(component: any, path: string): string[] {
  const errors: string[] = [];
 ...
 */
function sanitizeComponent(component: ComponentConfig): ComponentConfig {
  ...
  // Sanitize children
  if (Array.isArray(component.children)) {
    sanitized.children = component.children.map(child => 
      typeof child === 'string' ? sanitizeString(child) : sanitizeComponent(child)
    ) as ComponentConfig[];
  } else if (typeof component.children === 'string') {
 ...
 */
function sanitizeEventHandlers(events: Record<string, string>): Record<string, string> {
 ...
 */
export function validateDataSource(source: any): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
 ...
 */
export function createComponentBuilder(type: string) {
  ...
  return {
    ...
    build(): ComponentConfig {
      return sanitizeComponent(this as ComponentConfig);
    }
  };
}
