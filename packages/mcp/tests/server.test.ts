...
import { MCPServerConfig, MCPTool, MCPResource, MCPPrompt } from '../src/types';
import { z } from 'zod';
...
describe('MCPServer', () => {
  describe('Server Creation', () => {
    ...
    test('should_throw_error_on_invalid_server_config', () => {
      expect(() => {
        new MCPServer({} as MCPServerConfig);
      }).toThrow('Server name is required');

      expect(() => {
        new MCPServer({ name: '', version: '1.0.0' } as MCPServerConfig);
      }).toThrow('Server name is required');
    });
  });
