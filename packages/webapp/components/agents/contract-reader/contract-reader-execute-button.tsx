...
export default function ContractReaderExecuteButton() {
  ...
  const hasHubspotConnection = false;
  ...
  const handleExecute = async (provider: string) => {
    try {
      ...
      // Call the contract-reader API with a default limit of 100
      const response = await fetch(`/api/agents/contract-reader?limit=10&provider=${provider}`);

      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Failed to execute contract reader agent');
      }
