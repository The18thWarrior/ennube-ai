...
export const ApiKeyCreateCard: React.FC<ApiKeyCreateCardProps> = ({ onCreated, onClose }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-lg shadow p-6 w-full mx-auto">
      ...
      {generatedToken && (
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Generated Key</label>
          <div className="mt-1 flex items-center gap-2">
            <input readOnly value={generatedToken} className="flex-1 px-3 py-2 rounded outline outline-gray-300 dark:outline-gray-700 dark:bg-gray-800 dark:text-gray-100" />
            <button className="bg-gray-200 hover:bg-gray-300 text-gray-800 px-3 py-2 rounded" onClick={handleCopy}>Copy</button>
          </div>
          <div className="text-xs text-gray-500 mt-2">Make sure to copy this key now — it will not be shown again.</div>
        </div>
      )}
