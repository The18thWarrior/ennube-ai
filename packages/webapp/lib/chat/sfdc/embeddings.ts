...
export async function embedText(text: string): Promise<number[]> {
  ...
  const res = await embedMany({ model: openai.textEmbeddingModel('text-embedding-3-small'), values: [text] });
    if (Array.isArray(res.embeddings) && res.embeddings.length > 0) return res.embeddings[0] as number[];
    return [];
  } catch (error) {
...
export async function embedManyTexts(texts: string[]): Promise<number[][]> {
  ...
  try {
    // embedMany returns an array of vectors for the provided values.
    // Use a standard embedding model identifier so the embedder can resolve the proper provider.
    const res = await embedMany({ model: openai.textEmbeddingModel('text-embedding-3-small'), values: texts });
    if (Array.isArray(res.embeddings) && res.embeddings.length > 0) return res.embeddings as number[][];
    return [];
  } catch (error) {
    // Normalize errors to be easy to catch by callers/tests
    throw new Error(`Embedding failed: ${error instanceof Error ? error.message : String(error)}`);
...
export function createFieldText(field: {
  ...
  // Relationship name if present
  if (field.relationshipName && field.relationshipName.trim().length > 0) {
    parts.push(`relationship: ${field.relationshipName.trim()}`);
  }
...
export function createChildRelationshipText(
  child: {
    childSObject: string;
    field: string;
    relationshipName: string;
  ...
  // Join with separator optimized for embeddings
  return `${child.childSObject}.${child.field} (${child.relationshipName})`;
}
