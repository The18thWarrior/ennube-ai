...
import { AdobeOCRService } from './AdobeOCRService';
...
export class ContractExtractionService {
  private adobeOCRService: AdobeOCRService;
  ...
  constructor(
    ...
    if (!adobeClientId || !adobeClientSecret) {
      throw new ContractReaderError(
        'Adobe OCR credentials not provided',
        'MISSING_CREDENTIALS'
      );
    }
    
    this.adobeOCRService = new AdobeOCRService(adobeClientId, adobeClientSecret);
    
    // Initialize Salesforce service
    this.salesforceService = new SalesforceService(salesforceConnection);
          ...
          if (result && result.stream) {
            ...
            const patchedStream = new ReadableStream({
              start(controller) {
                ...
                function pump() {
                  reader.read().then(( value : ReadableStreamReadResult<any> ) => {
                    if (value.done) {
                      controller.close();
                      return;
                    }
  ...
  async extractFromSalesforce(
    ...
    try {
      ...
      throw new ContractReaderError(
        `Failed to extract contract from Salesforce: ${contractId}`,
        'SALESFORCE_EXTRACTION_ERROR',
        { contractId, error }
      );
    }
  }
  ...
  async processDocument(
    ...
    try {
      ...
      if (document.fileExtension.toLowerCase() === 'pdf' && !options.skipOCR) {
        ...
        extractedText = await this.adobeOCRService.processDocument(
          documentBuffer,
          document.title,
          {
  ...
  private async extractContractDataWithAI(
    text: string,
    options: ProcessingOptions
  ): Promise<ContractData> {
    try {
      if (!this.aiModel) {
        throw new AIExtractionError('AI model not initialized');
      }
...
Extract information from the file. Extract dates, names, terms. Do not output any information not present in the document.`;
  ...
  private parseAIResponse(response: string): ContractData {
    try {
      ...
      throw new AIExtractionError('Failed to parse AI response as JSON', { response, error });
    }
  }

  private validateContractData(data: ContractData): ContractData {
    try {
      return ContractDataSchema.parse(data);
    } catch (error) {
  ...
  private extractTextFromBuffer(buffer: Buffer, fileExtension: string): string {
