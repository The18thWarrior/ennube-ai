...
class SalesforceService {
    ...
    async getContractDocuments(contractId) {
        try {
            ...
            throw new index_js_1.SalesforceError(`Failed to retrieve contract documents for ID: ${contractId}`, { contractId, error });
        }
    }
    /**
     ...
     */
    async downloadDocumentContent(documentInfo) {
        try {
            ...
            if (!instanceUrl || !sessionId) {
                throw new index_js_1.SalesforceError('Invalid Salesforce connection - missing instance URL or session');
            }
            ...
            if (!response.ok) {
                throw new index_js_1.SalesforceError(`Failed to fetch document: ${response.status} ${response.statusText}`);
            }
            const arrayBuffer = await response.arrayBuffer();
            return Buffer.from(arrayBuffer);
        }
        catch (error) {
            throw new index_js_1.SalesforceError(`Failed to download document: ${documentInfo.title}`, { documentInfo, error });
        }
    }
    /**
    ...
    async getContract(contractId) {
      ...
      `;
            ...
            if (result.records.length === 0) {
                throw new index_js_1.SalesforceError(`Contract not found: ${contractId}`);
            }
            return result.records[0];
        }
        catch (error) {
            throw new index_js_1.SalesforceError(`Failed to retrieve contract: ${contractId}`, { contractId, error });
        }
    }
    async getContentDocumentLinks(contractId) {
        const query = `
      SELECT Id, 
             LinkedEntityId,
             ContentDocumentId
    ...
    async getContentDocuments(documentIds) {
    ...
    async getContentVersions(contentDocumentId) {
        const query = `
      SELECT Id, 
             Title, 
             FileExtension, 
    ...
    async searchContracts(criteria) {
      ...
      `;
            ...
            throw new index_js_1.SalesforceError('Failed to search contracts', { criteria, error });
        }
    }
}
