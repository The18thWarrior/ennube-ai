// === MultiRowValidator.test.tsx ===
// Created: 2025-07-19
// Purpose: Tests for MultiRowValidator component
...
describe('MultiRowValidator', () => {
  ...
  it('renders loading indicator when loading', () => {
    render(<MultiRowValidator sources={[sourceA, sourceB]} idKey="id" loading />);
    expect(screen.getByRole('status')).toBeInTheDocument();
  });

  it('shows message when no matching records', () => {
    render(<MultiRowValidator sources={[[], []]} idKey="id" />);
    expect(screen.getByText(/no matching records found/i)).toBeInTheDocument();
  });

  it('renders multiple comparison tables', () => {
    render(<MultiRowValidator sources={[sourceA, sourceB]} idKey="id" threshold={0.5} />);
