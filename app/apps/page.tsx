	...
	return (
		<main className="min-h-screen bg-neutral-950 text-white py-12 px-4">
			<div className="max-w-4xl mx-auto">
				...
				<p className="text-neutral-400 mb-8">Access a variety of AI-powered applications to enhance your productivity.</p>
				<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
					{apps.map((app, idx) => (
										<Link
											key={app.name}
											href={app.url}
											className="bg-neutral-900 rounded-xl shadow-lg p-6 flex flex-col items-center hover:scale-105 transition-transform border border-neutral-800 focus:outline-none focus:ring-2 focus:ring-blue-500"
											tabIndex={0}
										>
											<div className="w-16 h-16 mb-4 flex items-center justify-center rounded-full bg-neutral-800">
												<Image
													src={app.image}
													alt={app.name}
													width={48}
											...
											<div className="text-sm text-neutral-400 text-center mb-2">{app.description}</div>
											{/* <div className="text-xs text-neutral-500 text-center">By {app.author}</div> */}
										</Link>
					))}
				</div>
				<div className="flex justify-center mt-10">
					<button className="bg-neutral-800 text-white px-6 py-2 rounded-full hover:bg-neutral-700 transition">See more</button>
				</div>
			</div>
		</main>
	);
